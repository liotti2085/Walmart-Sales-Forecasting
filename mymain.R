...
test = test %>% 
  mutate(Weekly_Pred1 = as.double(Weekly_Pred1), Weekly_Pred2 = as.double(Weekly_Pred2), 
         Weekly_Pred3 = as.double(Weekly_Pred3))
...
naive_model<- function(train_ts, test_ts){
  ...
  train_ts <- na.seadec(train_ts, algorithm = "interpolation")
...
tslm_model<- function(train_ts, test_ts){
  ...
  for(j in 2:ncol(train_ts)){
    ...
    test[, j] <- forecast(model, num_forecasts)$mean
  }
...
mypredict <- function() {
  ...
  for (dept in test_depts) {
    ...
    #3rd
    pro <- product(train_dept_ts, test_dept_ts)
    test_month <- update_forecast(test_month, pro, dept, 3)
  }
